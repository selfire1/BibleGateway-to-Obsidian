#!/usr/bin/ruby
#----------------------------------------------------------------------------------
# This script is a modified version of Jonathan Clark's amazing BibleGateway passage lookup and parser to Markdown
# → https://github.com/jgclark/BibleGateway-to-Markdown/blob/master/bg2md.rb
# Modified last on 30.01.2021
#----------------------------------------------------------------------------------
...
opt_parser = OptionParser.new do |o|
  ...
  o.separator ''
  opts[:copyright] = false # automatically exclude copyright
  o.on('-c', '--copyright', 'Exclude copyright notice') do
    opts[:copyright] = false
  end
  opts[:headers] = false # automatically exclude headers
  o.on('-e', '--headers', 'Exclude editorial headers') do
    opts[:headers] = false
  end
  opts[:footnotes] = false # automatically exclude footnotes
  ...
  opts[:crossrefs] = false # automatically exclude cross-references
...
if opts[:numbering]
  ...
  passage.gsub!(%r{<sup class="versenum">\s*(\d+-?\d?)\s*</sup>}, '###### v\1 ')
...
# Change the red text for Words of Jesus to be bold instead # Do not change words of Jesus
passage.gsub!(%r{<span class="woj">(.*?)</span>}, '')
