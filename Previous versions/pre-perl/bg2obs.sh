...
# Navigation in the note
  ...
  text=$(echo $text | sed 's/#.*(#####\D[1]\D)/#$1/') # Delete headers again (Previously in Atom)
  text=$(echo $text | sed 's/######\s([0-9]\s|[0-9][0-9]\s|[0-9][0-9][0-9]\s)/\n\n###### v$1\n/') # Verse numbers
  text=$(echo $text | sed 's/\<crossref intro.*crossref\>//') # Delete crossreferences
  ...
  mv "$book.md" './Scripture ('"${translation}"')'
